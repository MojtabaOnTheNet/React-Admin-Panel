import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from "@/components/ui/table"
import { Card } from "@/components/ui/card"
import { X } from "lucide-react"
import { Switch } from "@/components/ui/switch"

const CommentsTableCard = () => {
  return (
    <Card className="w-full">
      <Table className="overflow-hidden text-lg">
        <TableHeader>
          <TableRow>
            <TableHead className="w-10 text-center">#</TableHead>
            <TableCell className="w-30 text-center">
              نام و نام خانوادگی
            </TableCell>
            <TableCell className="w-30 text-center">نوع نطر</TableCell>
            <TableCell className="w-30 text-center">برای</TableCell>
            <TableCell className="w-30 text-center">قسمتی از متن نظر</TableCell>
            <TableCell className="w-30 text-center">وضعیت</TableCell>
            <TableCell className="w-30 text-center">تاریخ</TableCell>
            <TableCell className="w-30 text-center">عملیات</TableCell>
          </TableRow>
        </TableHeader>
        <TableBody>
          <TableRow>
            <TableCell className="w-10 text-center font-medium">1</TableCell>
            <TableCell className="w-30 text-center">مجتبی علیزاده</TableCell>
            <TableCell className="w-30 text-center">نظر</TableCell>
            <TableCell className="w-30 text-center">محصول فلان</TableCell>
            <TableCell className="w-30 text-center">
              قسمتی از متن نظر برای محصول
            </TableCell>
            <TableCell className="w-30 space-x-2 text-center">
              <span>فعال</span>
              <Switch defaultChecked />
            </TableCell>
            <TableCell className="w-30 text-center">1404/04/10</TableCell>
            <TableCell className="w-30">
              <div className="flex flex-row-reverse justify-center gap-1">
                <X />
              </div>
            </TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </Card>
  )
}

export default CommentsTableCard
